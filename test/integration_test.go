...
var integrationTests = []struct {
	...
	{"list tags supplied during upload", tagListUploaded},
	{"replace file tags", tagReplace},
	{"update file tags", tagUpdate},
	{"clear file tags", tagClear},
...
func TestProjectAPIIntegration(t *testing.T) {
	...
	client, err := ucare.NewBearerClient(
		token,
		ucare.NewBearerConfig(ucare.WithRetry(&ucare.RetryConfig{MaxRetries: 3})),
	)
	if err != nil {
		t.Fatal(err)
	}
